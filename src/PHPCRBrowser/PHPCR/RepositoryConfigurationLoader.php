    ...
    public function getRepositories()
    {
        ...
        try {
            ...
            if (!is_array($repositories)) {
                throw new \InvalidArgumentException('An error occured during repositories parsing : repositories is not an array');
            }

            foreach ($repositories as $name => $repository) {
                if (!is_array($repository)) {
                    throw new \InvalidArgumentException('An error occured during repositories parsing : repository is not an array');
                }

                if (count(array_intersect(array_keys($repository), array('factory', 'parameters'))) != 2) {
                    throw new \InvalidArgumentException('An error occured during repositories parsing : missing factory or parameters');
                }

                if (!is_null($repository['parameters']) && !is_array($repository['parameters'])) {
                    throw new \InvalidArgumentException('An error occured during repositories parsing : parameters is not an array');
                }

                if (!array_key_exists($repository['factory'], $factories)) {
                    throw new \InvalidArgumentException('An error occured during repositories parsing : factory does not exist');
                }
                ...
                if(count(array_intersect(
                    ...
                    throw new \InvalidArgumentException('An error occured during repositories parsing : missing parameters');
                }
            ...
            throw new \InvalidArgumentException('An error occured during repositories parsing : factories or repositories does not exist');
        }
